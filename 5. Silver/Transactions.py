...
df_hosa=spark.read.parquet("/mnt/rcmdatalake03/bronze/hosa/transactions")

#Reading Hospital B departments data 
df_hosb=spark.read.parquet("/mnt/rcmdatalake03/bronze/hosb/transactions")
...
# MAGIC %sql
# MAGIC CREATE TABLE IF NOT EXISTS healthcare_rcm_catalog.silver.transactions (
...
# MAGIC MERGE INTO healthcare_rcm_catalog.silver.transactions AS target USING quality_checks AS source ON target.TransactionID = source.TransactionID
...
# MAGIC %sql
# MAGIC -- Insert new record and updated records to implement SCD Type 2
# MAGIC MERGE INTO healthcare_rcm_catalog.silver.transactions AS target USING quality_checks AS source ON target.TransactionID = source.TransactionID
# MAGIC AND target.is_current = true
...
# MAGIC %sql
# MAGIC Select * from healthcare_rcm_catalog.silver.transactions;